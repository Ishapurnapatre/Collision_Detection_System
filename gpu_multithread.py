...
def result_display_thread():
    ...
    while not video_end_event.is_set() or not display_frame_queue.empty():
        try:
            ...
            while not result_queue.empty():
                ...
                for box, track_id, confidence, class_name in zip(boxes_xywh, track_ids, confidences, class_names):
                    ...
                    if len(track_history[track_id]) >= 10 and len(previous_times[track_id]) >= 10:
                        ...
                        color = (0, 0, 255) if product > 15 else (255, 255, 255)  # Red if above threshold, white otherwise
            ...
            # Record GPU metrics
            gpu_utilization, gpu_memory_free, gpu_memory_used, gpu_temperature = get_gpu_utilization()
            if gpu_utilization is not None:
                gpu_times.append(time.time())
                gpu_utilizations.append(gpu_utilization)
                gpu_memory_frees.append(gpu_memory_free)
                gpu_memory_useds.append(gpu_memory_used)
                gpu_temperatures.append(gpu_temperature)
    ...
    plot_gpu_metrics()
    print_gpu_metrics_averages()  # Print GPU metrics averages
...
def plot_gpu_metrics():
    ...
    plt.subplot(2, 2, 1)
    plt.plot(gpu_times, gpu_utilizations, label='GPU Utilization (%)')
    plt.xlabel('Time')
    ...
    plt.subplot(2, 2, 2)
    plt.plot(gpu_times, gpu_memory_frees, label='GPU Memory Free (MB)')
    plt.xlabel('Time')
    plt.ylabel('Memory Free (MB)')
    plt.title('GPU Memory Free')
    plt.grid()

    plt.subplot(2, 2, 3)
    plt.plot(gpu_times, gpu_memory_useds, label='GPU Memory Used (MB)')
    plt.xlabel('Time')
    ...
    plt.subplot(2, 2, 4)
    plt.plot(gpu_times, gpu_temperatures, label='GPU Temperature (°C)')
    plt.xlabel('Time')
