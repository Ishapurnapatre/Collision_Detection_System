...
def initialize_streamlit_ui():
    ...
    with col_left:
        st.header("YOLOv8 Object Tracking")
...
def main():
    ...
    st.title("Collision Detection System")
    ...
    while cap.isOpened():
        if st.session_state.run and not st.session_state.pause:
            ...
            if success:
                ...
                st.session_state.stframe.image(annotated_frame, channels="BGR")
            ...
            if st.session_state.annotated_frame is not None:
                st.session_state.stframe.image(st.session_state.annotated_frame, channels="BGR")
            time.sleep(dt)
        else:
            break
