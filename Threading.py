...
def process_tracking_data(track_id, box, class_name, center, area, track_history, velocity_history, acceleration_history, previous_times, previous_centers, frame, threshold):
    try:
        ...
        print(f"Track ID: {track_id}, Class: {class_name}, Distance: {distance:.2f}m")
        ...
        track_history[track_id].append((box[0], box[1], box[2], box[3], distance))
        ...
        if len(track_history[track_id]) >= 10 and len(previous_times[track_id]) >= 10:
            ...
            text_y = int(box[1] - box[3]/2 - 110)
            cv2.putText(frame, f"Track ID: {track_id}", (int(box[0] - box[2]/2), text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
            text_y += 15
            cv2.putText(frame, f"Distance: {distance:.2f}m", (int(box[0] - box[2]/2), text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
            text_y += 15
            cv2.putText(frame, f"Velocity: {velocity:.2f}m/s", (int(box[0] - box[2]/2), text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
            text_y += 15
            cv2.putText(frame, f"Acceleration: {acceleration:.2f}m/s²", (int(box[0] - box[2]/2), text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
            text_y += 15
            cv2.putText(frame, f"Direction: {direction}", (int(box[0] - box[2]/2), text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        ...
        if results and hasattr(results[0], 'boxes') and results[0].boxes:
            ...
            if hasattr(boxes, 'xywh') and hasattr(boxes, 'id') and boxes.id is not None:
                ...
                for box, track_id, confidence, class_name in zip(boxes_xywh, track_ids, confidences, class_names):
                    ...
                    thread = threading.Thread(target=process_tracking_data,
                                              args=(track_id, box, class_name, center, area,
                                                    track_history, velocity_history, acceleration_history,
                                                    previous_times, previous_centers, frame, threshold))
