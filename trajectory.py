            ...
            if hasattr(boxes, 'xywh') and hasattr(boxes, 'id') and boxes.id is not None:
                ...
                for box, track_id, class_name in zip(boxes_xywh, track_ids, class_names):
                    ...
                    if len(self.vehicle_histories[track_id]) > 1:
                        ...
                        cv2.line(self.mask, (prev_center_x, prev_center_y), (center_x, center_y), self.colors[track_id % 100].tolist(), 2)
                        ...
                        # Display ID, confidence, and direction
                        cv2.putText(annotated_frame, f"ID: {track_id} Conf: {conf:.2f}", (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                        cv2.putText(annotated_frame, f"Direction: {most_common_direction}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
